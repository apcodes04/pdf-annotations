class AnnotationManager {
    constructor(historyManager) {
        ...
        this.applyBoxBtn = document.getElementById('applyBoxBtn');
    ...
    setupGlobalListeners() {
        document.addEventListener('pointerdown', (e) => {
            ...
            if (!e.target.closest('.text-box') && !e.target.closest('.image-box') && !e.target.closest('.dont-invert-box') && !e.target.closest('#toolbar')) {
                if (this.cropperInstance) this.applyCrop();
                this.setActive(null);
            }
        });
            ...
            if (this.currentMode === 'text' || this.currentMode === 'image' || this.currentMode === 'dont-invert') {
                ...
                if (pageContainer && !e.target.closest('.text-box') && !e.target.closest('.image-box') && !e.target.closest('.dont-invert-box')) {
                    ...
                    if (this.currentMode === 'text') {
                        ...
                        this.startDrawingDontInvertBox(pageContainer, x, y, e.pointerId);
                    }
                }
            }
        });
    ...
    setupFormattingListeners() {
        ...
        addListener(this.applyBoxBtn, 'click', () => {
            this.setActive(null);
        });

        addListener(this.deleteBoxBtn, 'click', () => {
            if (this.activeElement) {
                const ann = this.getAnnotationByElement(this.activeElement);
                if (ann) {
                    const cmd = new DeleteAnnotationCommand(this, ann);
    ...
    updateFormattingToolbar(ann) {
        ...
        if (ann.type === 'dont-invert') {
            ...
            this.applyBoxBtn.style.display = 'block';
            this.deleteBoxBtn.style.display = 'block';
            ...
            this.applyCropBtn.style.display = 'none';
            this.cancelCropBtn.style.display = 'none';
            this.applyBoxBtn.style.display = 'none';
            this.deleteBoxBtn.style.display = 'block';
    ...
    startDrawingDontInvertBox(pageContainer, startX, startY, pointerId) {
        const layer = pageContainer.querySelector('.annotation-layer');
        const box = document.createElement('div');
        box.className = 'dont-invert-box active';
        ...
        pageContainer.setPointerCapture(pointerId);
        ...
        const onUp = (e) => {
            pageContainer.releasePointerCapture(pointerId);
            pageContainer.removeEventListener('pointermove', onMove);
            pageContainer.removeEventListener('pointerup', onUp);
            
            if (box.offsetWidth < 10 || box.offsetHeight < 10) {
        ...
        pageContainer.addEventListener('pointermove', onMove);
        pageContainer.addEventListener('pointerup', onUp);
    }
